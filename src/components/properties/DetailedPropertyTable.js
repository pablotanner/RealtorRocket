...
const DetailedPropertyTable = ({ properties }) => {
    ...
    const Header = () => {
        return (
            <div style={{gridTemplateColumns: "minmax(250px, 1fr) 1fr 1fr 1fr 50px" }} className="bg-white rounded-2xl border-gray-100 border-2 p-4 w-full grid grid-cols-5 gap-8 overflow-auto font-500 text-gray-800">
    ...
    const PropertyRow = ({ property, key }) => {
        return (
            <div key={key} style={{gridTemplateColumns: "minmax(250px, 1fr) 1fr 1fr 1fr 50px" }} className="bg-white rounded-2xl border-gray-100 border-2 p-4 w-full grid grid-cols-5 gap-8 overflow-auto">
                ...
                <div className="flex flex-row gap-2 flex-wrap h-full items-center">
                    {property.units.map((unit, index) => {
